...
документа — отдельная исследовательская задача, вне рамок этого
документа (по решению в ходе брейнсторминга).
...
- Исследовательская задача: карта полей и покрытие по годам, механика
  связи версий документа (обе — отдельные шаги Этапа 1 продуктового
  ТЗ, не включённые сюда).
- Этап 2 продуктового ТЗ: расширение сборщика на `Document[]`,
  `Report[]`, `Category`, дополнительные вкладки `comment_channel`
  (1/3/4/7/8/9/10), после того как для каждого будет решено, что
  именно собирать.
