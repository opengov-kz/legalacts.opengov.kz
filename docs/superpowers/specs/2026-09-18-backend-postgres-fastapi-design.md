...
- `first_seen_at`, `last_checked_at`, `discovered_at`, `processed_at`
  остаются `String` (ISO-8601 текст, как пишет `now_iso()` сейчас) —
  **не** `TIMESTAMPTZ`. Причина решения (уточнено на этапе
  планирования): лексикографическая сортировка/сравнение ISO-8601
  строк эквивалентна хронологическому порядку, так что поведение
  (`ORDER BY discovered_at`, сравнение с порогом устаревания в
  `requeue_stale_*`) не меняется; при этом не нужно вводить
  конвертацию `str ↔ datetime` на всём пути записи (`run.py`,
  `store.py`, `queue.py`) и в тестовых фикстурах — риск рассинхронизации
  форматов того не стоит при нулевой функциональной выгоде на этом
  этапе. Аналитические выборки с группировкой по дате (раздел 4)
  используют явный `CAST(... AS timestamptz)`/`date_trunc` на уровне
  SQL-запроса, а не тип колонки.
