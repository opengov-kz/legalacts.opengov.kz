...
- `QualityEvent`: события группы A не экспонируются через API v1 в этой итерации (только сбор в БД, как `DocumentVersion`/`Report`/`Category`); нет дедупликации — при стабильной проблеме на одном акте события накапливаются на каждом обходе.
