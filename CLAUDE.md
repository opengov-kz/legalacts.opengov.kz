...
Флаг `--limit` ограничивает число обработанных записей очереди за один запуск (list-страница или карточка документа = 1 запись); без флага обход идёт до полного исчерпания очереди. Повторные запуски резюмируемы — очередь не пересоздаётся, если в ней уже есть необработанные записи, а `requeue_stale_documents`/`requeue_stale_lists` переоткрывают записи (`status='done'`) старше 7 дней для докачки; `requeue_stale_errors` по тому же порогу переоткрывает записи со `status='error'`, но только пока `consecutive_errors` не достиг предела `MAX_ERROR_RETRIES` (5) — после этого запись остаётся в `error` навсегда, требуя ручного вмешательства.
...
- `backend/scraper/queue.py` — управление очередью обхода (`CrawlQueue`): `enqueue`, `next_pending`, `any_exist`, `section_for`, `mark_done`, `mark_error`, `requeue_stale_documents`, `requeue_stale_lists`, `requeue_stale_errors`. Единственный источник истины о том, что уже обработано (двигатель резюмируемости).
- `backend/scraper/store.py` — сохранение актов и комментариев в БД (upsert): `upsert_legal_act` (get-or-create для `GovernmentBody`/`ActType`; вставляет новую строку `LegalActSnapshot`, только если изменились статус/сроки/счётчики/хэш содержимого — идемпотентно для неизменного повторного обхода), `upsert_comments` (принимает `comment_channel`). Сохраняет `first_seen_at`, обновляет `last_checked_at`; kk-поля не затираются, если при очередном обходе не пришли — включая `raw_html_kk`, который в этом случае переносится из последнего снапшота. Также сохраняет `views_count` (счётчик просмотров источника, с 2026-09-24) — НЕ входит в `SNAPSHOT_TRIGGER_FIELDS`: просмотры меняются почти на каждом обходе, включение туда завалило бы `LegalActSnapshot` почти-дублирующимися строками на каждый повторный обход. `upsert_legal_act` также пишет `QualityEvent`-записи для четырёх проверок из раздела 22.3 продуктового ТЗ (группа A — на уровне одной записи): битый URL (`url` не начинается с `BASE_URL`), невалидная дата (`created_date`/`discussion_end_date` не парсятся форматом `DD/MM/YYYY`), конец обсуждения раньше начала, флип счётчика (`comments_total`/`likes_count`/`dislikes_count`) в/из `NULL` между последовательными обходами одного акта, новое значение `status` (глобально впервые для всего датасета, сверяется с `known_status_values`). Сравнение `comments_total` с фактически собранным числом комментариев реализовано отдельно — `store.record_comments_total_mismatch(session, legal_act_id, comments_total, now)`, вызывается из `run.py`'s `process_document_entry` после сбора всех каналов комментариев (не из `upsert_legal_act`, т.к. на момент upsert'а комментарии ещё не собраны); считает `Comment` по всем каналам для акта, пишет `QualityEvent(event_type="comments_total_mismatch")` при любом расхождении (без порога). `store.record_unrecognized_html_structure(session, legal_act_id, template_recognized, now)` — там же, сразу после `upsert_legal_act`; `template_recognized` приходит от `document_page.parse_document_page` (`True`, если совпал `.view-npa` или `.blog-item`). `store.record_list_total_pages_change(session, canonical_url, total_pages, now)` — в `process_list_entry`/`process_category_list_entry`, сравнивает `total_pages` с последним известным для того же (канонизированного к `page=1`) URL в `list_page_totals`; при уменьшении пишет `QualityEvent` с `legal_act_id=None`, при любом исходе обновляет baseline на новое значение. Все три проверки — без порога и без исключений по разделу (`arv` не требует особой обработки: у него уже узнаваемый шаблон `.blog-item`). Раздел 22.3 продуктового ТЗ закрыт полностью, кроме `response time` (вне v1) и обнаружения новых типов/категорий как отдельных событий: новые `doc_type`/`government_body` молча создают строку в справочнике без `QualityEvent` (детектируемо запросом, но не событийно), а новые категории источника вообще не обнаруживаются — `CATEGORIES` в `run.py` статический список из 25 значений, категория, отсутствующая в нём, никогда не будет обойдена.
...
Начиная с Этапа 2, `run.py` также обходит категории/тематики: `CATEGORIES` (статический список из 25 `(id, name)`, используется только для построения 150 стартовых URL — 6 существующих seed-записей × 25 категорий) и `process_category_list_entry` (новый `page_type="category_list"`, переиспользует `list_page.parse_list_page`/`parse_total_pages` и пагинацию `_set_page_param`; парсит имя категории живьём через `list_page.parse_category_name`, с откатом на `CATEGORY_NAMES`, если на странице категория не нашлась). Если карточка ссылается на акт, которого ещё нет в `legal_acts` — связь не создаётся, карточка пропускается (акт будет найден штатным обходом `list`/`Arvlist`, связка добьётся повторным обходом). `category_list` обрабатывается в `run()` только когда очереди `list` и `document` пусты — третий, самый низкий приоритет. `requeue_stale_lists` переоткрывает устаревшие записи обоих типов (`list` и `category_list`).
...
- `DocumentVersion` (версии-снапшоты через `/application/viewcardhistory`) собираются только на ru, без вложенных приложенных файлов и без экспонирования через API v1 (пока только для сбора/архива). Запись — insert-once: если сбор цепочки версий прервётся сетевой ошибкой посередине (вся запись очереди documents уйдёт в `error`), уже собранные версии останутся в БД; `error`-записи теперь переоткрываются автоматически через `requeue_stale_errors` (см. ниже) — версии-цепочки по-прежнему увеличивают число сетевых запросов на документ и потому чуть повышают вероятность столкнуться с ошибкой, но она больше не остаётся висеть навсегда.
- `Report` (сырой HTML отчёта об итогах обсуждения через `/report?id=<external_id акта>`) собирается только на ru, без разбора строк таблицы на структурированные пары комментарий/ответ и без экспонирования через API v1. Запись — insert-once, максимум одна на акт: сохраняется, только если ответ `/report` вернул `200` с непустым телом (`!= 404` одной проверки недостаточно — WAF/rate-limit `403` или редирект на страницу ошибки иначе записались бы в БД навсегда, без возможности повторной проверки).
- `Category`: связь с актом создаётся, только если акт уже был обойдён штатным `list`/`Arvlist`-обходом на момент обработки категорийной страницы; для актов, обнаруженных позже, связка появится только на следующем полном обходе категории (`requeue_stale_lists`, порог `STALE_AFTER_DAYS` = 7 дней) — до этого момента акт существует в базе без связей с категориями, в которых он на самом деле состоит. Приоритет `category_list` в цикле `run()` — самый низкий (после `list` и `document`), поэтому на большом объёме очереди (как на этом сайте) первый проход по категориям может начаться существенно позже, чем кажется из одного факта «сборщик запущен» — точная задержка зависит от текущего размера очереди `list`/`document` на момент запуска.

- Очередь (`crawl_queue`): записи со статусом `error` переоткрываются автоматически через `requeue_stale_errors` — по тому же порогу `STALE_AFTER_DAYS` (7 дней), что и `done`-записи, но с пределом попыток: если `consecutive_errors` записи достиг `MAX_ERROR_RETRIES` (5), она больше не переоткрывается автоматически и остаётся в `error` навсегда — такую запись видно в `GET /crawl/status` (`recent_errors`) и чинят вручную. `consecutive_errors` — отдельное поле от `attempts` (последнее — счётчик за всю жизнь записи, включая успешные обходы; `consecutive_errors` сбрасывается в 0 при каждом `mark_done` и специально считает подряд идущие неудачи, иначе предел «N попыток» ошибочно сработал бы на записи с долгой историей успешных повторных обходов).
- `QualityEvent`: события группы A не экспонируются через API v1 в этой итерации (только сбор в БД, как `DocumentVersion`/`Report`/`Category`); нет дедупликации — при стабильной проблеме на одном акте события накапливаются на каждом обходе. `counter_null_flip` сегодня фактически недостижим через реальный вызов из `run.py` — `parse_document_page` нормализует отсутствующий счётчик в `0`, а не в `NULL`, так что проверка реагирует только на синтетические данные в тестах. Коллапс счётчика в `0` при поломке селектора — предмет будущей группы B (агрегатные проверки), не этой итерации.
